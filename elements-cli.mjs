...
// הוסף את זה ליד ההגדרות האחרות בתחילת הקובץ
const LIVERELOAD_PORT = parseInt(process.env.LIVERELOAD_PORT || '35729');
...
function upgrade(server) {
    const wss = new WebSocketServer({ 
        server: server,
        path: sanitize(`/${argv['base-path']}`)  // הוספנו את זה
    });
...
function watch(filePath, server) {
    ...
    return watcher.on('all', (event, path) => {
        ...
        try {
            // שלח הודעה לכל הלקוחות המחוברים
...
// עדכון הנתיב של ה-favicon להשתמש ב-base-path
app.use(
    sanitize(`/${argv['base-path']}/favicon`),
    (req, res, next) => {
        console.log(`Favicon request: ${req.url}`);
        next();
    },
    express.static(path.join(__dirname, 'favicon'))
);

// השאר את ההגדרה הקיימת של הנתיב הסטטי
app.use(
...
app.get(
    ...
    (req, res) => {
        ...
        res.render('index', {
            ...
            'elements-css': `${baseHref}styles.min.css`,
            'elements-js': `${baseHref}web-components.min.js`,
            layout: false,
            'livereload-js': argv.watch ? `${baseHref}livereload.js` : undefined,
...
const wss = argv.watch ? upgrade(server) : undefined;
